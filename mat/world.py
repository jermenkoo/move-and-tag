...
class World:
    ...
    def findClusters(self, G):
        ...
        for e in G.edges():
            w = G[e[0]][e[1]]['weight']
            averageEdgeWeight += w / l

        for e in G.edges():
            if G[e[0]][e[1]]['weight'] < averageEdgeWeight:
                score[e[0]] += G[e[0]][e[1]]['weight'] - averageEdgeWeight
                score[e[1]] += G[e[0]][e[1]]['weight'] - averageEdgeWeight
        ...
        for s in scores:
            ...
            cluster = []
            for s1 in scores:
                if G[s[0]][s1[0]]['weight'] < averageEdgeWeight / 4:
                    cluster.append(s1[0])
                    scores.remove(s1)
